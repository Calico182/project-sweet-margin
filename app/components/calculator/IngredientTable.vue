  ...
  <section class="panel">
    <header class="panel__header">
      <div>
        ...
        <p class="panel__text">Add each ingredient with recipe and purchase details.</p>
      </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="ingredient in modelValue" :key="ingredient.id">
            ...
            <td><input v-model.number="ingredient.purchaseQuantity" type="number" min="0" step="0.01" /></td>
            <td>
              <select v-model="ingredient.purchaseUnit">
                <option v-for="unit in units" :key="unit" :value="unit">{{ unit }}</option>
              </select>
            </td>
            <td><input v-model.number="ingredient.purchasePrice" type="number" min="0" step="0.01" /></td>
